...
#     exit 1
# fi
environment="Aries-Cloudagent-Python"
...
function __test_kob_validate
{
    ...
    local return_val=$(__kobman_validate_Aries-Cloudagent-Python "$envirnoment")
...
function __test_kob_run
{
    ...
    if [[ $test_status == "success" ]]; then
        __kobman_echo_green "test-kob-Aries-Cloudagent-Python success"
    else
        __kobman_echo_red "test-kob-Aries-Cloudagent-Python failed"
    fi
}
__test_kob_run
